    ...
    <section id="proyectos" className="py-24">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((p) => {
            const accent = p.color === "blue" ? "blue" : "violet"
            const borderAccent =
              accent === "blue"
            ...
            return (
              <div
                key={p.name}
                className={`glass rounded-2xl p-8 flex flex-col gap-6 transition-all duration-300 ${borderAccent} hover:bg-white/[0.06]`}
              >
                {/* Icon + name */}
                ...
                {/* Description */}
                <p className="text-slate-300 leading-relaxed text-sm">{p.description}</p>

                {/* Routes */}
                <div>
                  <p className="text-xs text-slate-600 uppercase tracking-wider mb-2">Rutas</p>
                  <div className="flex flex-wrap gap-2">
                    {p.routes.map((r) => (
                      <span
                        key={r}
                        className={`font-mono text-xs px-2.5 py-1 rounded-full border ${routeColor}`}
                ...
                {/* Stack */}
                <div>
                  <p className="text-xs text-slate-600 uppercase tracking-wider mb-2">Stack</p>
                  <div className="flex flex-wrap gap-2">
                    {p.stack.map((t) => (
                ...
                {p.url ? (
                  <a
                    ...
                    className="mt-auto inline-flex items-center gap-1.5 text-sm text-blue-400 hover:text-blue-300 transition-colors"
                  >
                    Ver en Vercel
                  ...
                  <span className="mt-auto text-sm text-slate-600 italic">Integrado en la plataforma unificada</span>
                )}
              </div>
            )
          })}
        ...
        {/* Fusion callout */}
        <div className="mt-8 glass rounded-2xl p-8 border border-white/[0.06]">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-violet-500/20 border border-white/[0.08] flex items-center justify-center flex-shrink-0">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                ...
                <code
                  className="text-blue-300 bg-blue-500/10 px-1.5 py-0.5 rounded"
                  style={{ fontFamily: "Azeret Mono, monospace" }}
                >
                  /twin
                </code>
